...
## Wires the sprite scene's full-area targeting [Button] ([code]CombatCharacterSprite.button[/code]).
func _wire_combat_sprite_targeting(sprite_instance: Node, combatant: CombatantData) -> void:
	...
	var target_btn: Button = sprite.button
	target_btn.pressed.connect(_on_combatant_clicked.bind(combatant))
	target_btn.mouse_entered.connect(_on_combat_sprite_hover_entered.bind(combatant))
	target_btn.mouse_exited.connect(_on_combat_sprite_hover_exited)
	combatant_clickable_areas[combatant] = target_btn
